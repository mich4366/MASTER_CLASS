...
view(TPM_zero)
#Square brackets allow for function and go into row column space
# Wow so almost half of the genes measured had no expression
# We like check for 0s as many 0s or NAs can throw off statiscial analysis 
...
#Removing zeros from the TPM file. in function is a left join, exclamation part to signify not to select gene_id
view(TPM_not_zero)
...
hist(TPM_filtered$WT_0_1, xlim = c(0,300), breaks = 1000)

# Lets compare means 
mean(TPM_filtered$WT_0_1)
...
#Average abundance of genes across all time points.
